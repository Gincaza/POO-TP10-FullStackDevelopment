...
class TestRegisterRoute(unittest.TestCase):
    ...
    @patch.object(database_context, 'insert_empregado')
    @patch.object(database_context, 'verify_empregado')
    def test_register_erro_verificacao_apos_registro(self, mock_verify_empregado, mock_insert_empregado, mock_get_empregado):
        # Mocking que o nome de usuário não está em uso
        mock_get_empregado.return_value = None
        # Mocking uma verificação falha após inserção
        mock_verify_empregado.return_value = False
...
class TestRegistrarPedidoRoute(unittest.TestCase):
    ...
    @patch.object(database_context, 'get_hamburguer')
    @patch.object(database_context, 'get_cliente')
    def test_registrar_pedido_cliente_nao_encontrado(self, mock_get_cliente, mock_get_hamburguer):
        # Mocking a resposta do banco de dados para hambúrguer
        mock_get_hamburguer.return_value = ["Hamburguer Teste", "Ingredientes Teste", "10.0"]
        # Mocking que o cliente não foi encontrado
        mock_get_cliente.return_value = None
    ...
    def test_registrar_pedido_dados_incompletos(self):
        dados = {
            "id_cliente": 1,
            "nome_cliente": "Cliente Teste",
            "nome_hamburguer": "Hamburguer Teste",
            # Quantidade, tamanho e data_hora faltando
        }

        response = self.client.post('/pedido', json=dados)
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.get_json(), {"erro": "Dados incompletos"})
